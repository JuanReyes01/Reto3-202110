...
public class Modelo {
	 ...
	 */
	private ArregloDinamico<Reproduccion> lista;
	...
	public Modelo()
	{   
		lista = new ArregloDinamico<Reproduccion>();
	...
	public String cargar() throws ParseException, IOException{
		...
		for (CSVRecord record : records) {
			...
			if(!instrumentalness.equals("instrumentalness")){
				...
				Reproduccion nuevo = new Reproduccion(caracteristicas,Double.parseDouble(loudness), Double.parseDouble(tempo),(int) Double.parseDouble(mode), (int) Double.parseDouble(key), artist_id, tweet_lang, track_id, fechaPu, lang, time_zone, (int) Double.parseDouble(user_id), (int) Double.parseDouble(id)); 
				lista.addLast(nuevo);
	...
	public String req1(String c, double min, double max){			
		...
		RBT<Double, ArregloDinamico<Reproduccion>> arbol = new RBT<Double, ArregloDinamico<Reproduccion>>();
		for(int i=1;i<=lista.size();i++){
			Reproduccion nuevo = lista.getElement(i);					
		...
		ILista<ArregloDinamico<Reproduccion>> totales = arbol.valuesInRange(min, max);
		ArregloDinamico<Reproduccion> todos = new ArregloDinamico<Reproduccion>();
		for(int i=1;i<=totales.size();i++){
			ArregloDinamico<Reproduccion> lista = totales.getElement(i);
			for(int j=1; j<=lista.size();j++){
				todos.addLast(lista.getElement(j));
			}
		}
		TablaHashSeparateChaining<String, Reproduccion> unicos = new TablaHashSeparateChaining<>(todos.size(), 1.5);
		for(int i=1; i<todos.size();i++){
			Reproduccion nuevo = todos.getElement(i);
			String key = nuevo.darArtist_id(); 
			Reproduccion aux2 =  unicos.get(key);
			if(aux2 == null){				
				unicos.put(key, nuevo);
			}		
		}
		ArregloDinamico<Reproduccion> v = (ArregloDinamico<Reproduccion>) unicos.valueSet();
		int x = unicos.size();
		String res = "++++++++++Req1 Resultados++++++++++"+"\n"+c+" con los valores entre: "+min+" y "+max+"\n Reproducciones totales "+todos.size()+" \n Artistas unicos: "+v.size();
		return res;
	}

	public ArregloDinamico<Reproduccion> req2(double minE, double maxE, double minD, double maxD){
		TablaHashSeparateChaining<String, Reproduccion> unicos = new TablaHashSeparateChaining<>(lista.size(), 1.5);
		for(int i=1; i<lista.size();i++){
			Reproduccion nuevo = lista.getElement(i);
			String key = nuevo.darArtist_id(); 
			Reproduccion aux2 =  unicos.get(key);
		...
		RBT<Double, ArregloDinamico<Reproduccion>> arbol = new RBT<Double, ArregloDinamico<Reproduccion>>();
		ArregloDinamico<Reproduccion> val = (ArregloDinamico<Reproduccion>) unicos.valueSet();
		...
		ILista<ArregloDinamico<Reproduccion>> totales = arbol.valuesInRange(minE, maxE);
		ArregloDinamico<Reproduccion> todos = new ArregloDinamico<Reproduccion>();
		for(int i=1;i<=totales.size();i++){
			ArregloDinamico<Reproduccion> lista = totales.getElement(i);
			for(int j=1; j<=lista.size();j++){
	...
	public String req4(String l){
		...
		for(int i=0;i<gnero.length;i++){
			...
			//Rangos
			ILista<ArregloDinamico<Reproduccion>> nuevaLista = arbol.valuesInRange(num.getElement(1), (num.getElement(2)));			
			//Arreglar el arreglo
			ArregloDinamico<Reproduccion> listaArreglada = new ArregloDinamico<Reproduccion>();
			for(int j=1;j<=nuevaLista.size();j++){
				ArregloDinamico<Reproduccion> todos = nuevaLista.getElement(j);
			...
			TablaHashSeparateChaining<String, Reproduccion> artistas = new TablaHashSeparateChaining<>(cantidadCanciones, 1.5);
			for(int j=1; j<=listaArreglada.size();j++){
				Reproduccion nuevo = listaArreglada.getElement(j);
				String key = nuevo.darArtist_id(); 
				Reproduccion aux2 =  artistas.get(key);
				if(aux2 == null){				
	...
	public String Req5(String horaInicio, String horaFinal){
		...
		RBT<String, ArregloDinamico<Reproduccion>> arbolHoras = new RBT<>();
		//Generar el arbol de horas/fechas
		for(int i=1;i<=lista.size();i++){
			Reproduccion nuevo = lista.getElement(i);		
			String llave = nuevo.darCreated_at().getHours()+":"+nuevo.darCreated_at().getMinutes(); 
			ArregloDinamico<Reproduccion> valor = arbolHoras.get(llave);		    																	   
			if(valor == null){
				ArregloDinamico<Reproduccion> v = new ArregloDinamico<Reproduccion>();
				v.addLast(nuevo);
		...
		ILista<ArregloDinamico<Reproduccion>> reproduccionesEnIntervalo = arbolHoras.valuesInRange(horaInicio, horaFinal);
		//De la lista obtener los valores en listas individuales no anidadas
		ArregloDinamico<Reproduccion> listaArreglada = new ArregloDinamico<>();
		for(int i=1;i<=reproduccionesEnIntervalo.size();i++){
			ArregloDinamico<Reproduccion> listaAnidada = reproduccionesEnIntervalo.getElement(i);
		...
		RBT<Double, ArregloDinamico<Reproduccion>> arbolTempo= new RBT<Double, ArregloDinamico<Reproduccion>>();
		for(int i=1;i<=listaArreglada.size();i++){
			Reproduccion nuevo = listaArreglada.getElement(i);		
			double llave = nuevo.darTempo(); 
			ArregloDinamico<Reproduccion> valor = arbolTempo.get(llave);		    																	   
			if(valor == null){
				ArregloDinamico<Reproduccion> v = new ArregloDinamico<Reproduccion>();
				v.addLast(nuevo);
		...
		int valMax = 0;
		ArregloDinamico<Reproduccion> mayor = new ArregloDinamico<>();
		String generoMax = "";
		for(int i=1; i<=gnros.size();i++){
			...
			ILista<ArregloDinamico<Reproduccion>> listaTempo = arbolTempo.valuesInRange(tiempos.getElement(1), tiempos.getElement(2));
			//Arrelar la lista
			ArregloDinamico<Reproduccion> listaArregladaTempo = new ArregloDinamico<>();
			for(int k=1;k<=reproduccionesEnIntervalo.size();k++){
				ArregloDinamico<Reproduccion> listaAnidada = reproduccionesEnIntervalo.getElement(k);
				for(int j=1;j<=listaAnidada.size();j++){
